/**
 * Created by weidong on 16/1/6.
 */
define(['scripts/controller/controller', '../../model/inventory/adjustStatusModel'], function (controller, adjustStatusModel) {
    "use strict";
    controller.controller('inventoryAdjustmentStatusController',
            ...
            function ($scope, $rootScope, $sync, $url, wmsDataSource, $filter) {
                var url = $url.inventoryAdjustUrl,
                    columns = [
                        ...
                        { title: '调整原因', field: 'resonCode', align: 'left', width: "150px", template:WMS.UTILS.codeFormat('resonCode', 'StatusAdjustReason')},
                    ...
                    detailBaseColumns = [
                        {filterable: false, title: '库区', field: 'zoneId', align: 'left', width: "100px",template:WMS.UTILS.zoneNoFormat},
                        {filterable: false, title: '货位', field: 'locationId', align: 'left', width: "100px", template:WMS.UTILS.locationFormat},
                ...
                var disableAry = ['skuItemName','skuBarcode','skuId','newInvStatusCode'],
                  hiddenAry = ['skuDiv i'],
                  clearValueAry = ['skuItemName','skuBarcode','skuId'];
                $scope.isflag = 'true';
                $scope.adjustDetailOptions = function(dataItem){
                    ...
                    return WMS.GRIDUTILS.getGridOptions({
                        dataSource:wmsDataSource({
                            ...
                            schema:{
                                model: adjustStatusModel.adjustDetail
                            },
                            pageSize: 5
                        }),
                        toolbar: toolbar,
                        ...
                        save: function(e){
                            ...
                            $scope.isflag = 'true';
//                            model.dirty = false;
                            if ((!model.originalInvStatusCode == true) || model.originalInvStatusCode == "") {
                                kendo.ui.ExtAlertDialog.showError("该货位没有库存,不允许调整状态!");
                                e.preventDefault();
                                $scope.isflag = 'false';
                                return;
                            }
                            ...
                            if (model.newInvStatusCode === model.originalInvStatusCode) {
                                kendo.ui.ExtAlertDialog.showError("原库存状态和新状态不能相同!");
                                e.preventDefault();
                                $scope.isflag = 'false';
                                return;
                            }
                            ...
                            if ($('#skuBarcode').val() != '') {
                                ...
                                if ($scope.isflag == 'true') {
                                    $scope.isflag = $scope.submitLocation(e.sender, e.model, false, "");
                                }
                                e.preventDefault();
                            }
